...
```json
{
  ...
  "schedules": {
    "weekday": {
      "label": "工作日班次（周一 ~ 周五）",
      "notes": {
        "fromLantai": "早间段 7:20-9:40 循环发车，每 10 分钟一班",
        "toLantai": "早间段 7:30-9:50 循环发车，每 10 分钟一班"
      },
      "fromLantai": [
    ...
    "weekend": {
      ...
      "notes": {
        "fromLantai": "早间段 7:20-9:40 为固定班次（其中 7:40 开往纪忠楼）",
        "toLantai": "早间段 7:30-9:50 为固定班次；全程经北门转盘"
      },
      "fromLantai": [
...
| `schedules.*.notes` | 按方向分别显示的说明（优先于 `directions[].note`），可省略 |
...
- 原表的三条线路（北门线、文学院线、纪忠楼线）已做成页面上的线路标签；原表脚注「晚间因南师附中下课，兰台回北门时间存在不确定性」放在对应方向说明里。
